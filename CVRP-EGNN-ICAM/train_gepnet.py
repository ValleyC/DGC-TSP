"""
GEPNet Training Script for CVRP.

CVRP-EGNN-ICAM: E(2)-Equivariant partition + ICAM sub-solver.

Key innovations over UDC:
1. EGNN backbone: No coordinate_transformation needed
2. Invariant features by construction
3. State-aware partition with depot, first, last, visited embeddings
"""
...
sys.path.insert(0, "..")  # For CVRProblemDef
...
trainer_params = {
    ...
    'validation_test_episodes': 128,
    'validation_test_batch_size': 4,
    'validation_aug_factor': 50,
    # Validation data files (optional)
    # 'data_load_500': 'path/to/cvrp500_validation.txt',
    # 'data_load_1000': 'path/to/cvrp1000_validation.txt',
}

logger_params = {
    'log_file': {
        'desc': 'gepnet_cvrp_n500_n1000',
        'filename': 'log.txt'
    }
}
...
def main():
    ...
    logger.info('=' * 60)
    logger.info('GEPNet: E(2)-Equivariant Partition Network for CVRP')
    logger.info('=' * 60)
