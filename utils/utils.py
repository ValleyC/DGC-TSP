"""
Utility functions for GEPNet training.

Based on UDC's utils with simplifications.
"""
...
class TimeEstimator:
    ...
    def get_est(self, count, total):
        ...
        remain = total - count
        remain_time = elapsed_time * remain / (count - self.count_zero) if (count - self.count_zero) > 0 else 0
...
def copy_all_src(dst_root):
    """Copy source files to result folder for reproducibility."""
    ...
    for item in sys.modules.items():
        ...
        if hasattr(value, '__file__') and value.__file__:
            ...
            if execution_path in src_abspath:
